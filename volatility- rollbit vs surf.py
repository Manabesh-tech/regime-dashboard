...
class ExchangeAnalyzer:
    ...
    def __init__(self):
        ...
        self.tick_duration_seconds = 0.5
    ...
    def _calculate_variance(self, prices):
        ...
        try:
            ...
            if pd.isna(variance) or np.isinf(variance):
                return 0.0001  # Default small value
            
            return variance
        except Exception as e:
            return 0.0001  # Return a reasonable default value
    
    def _calculate_annualized_variance(self, prices):
        """
        Calculate annualized variance of returns.
        Assumes each tick is approximately 500ms (0.5 seconds).
        """
        try:
            # Calculate returns (percentage changes)
            returns = prices.pct_change().dropna()
            ...
            # For 500ms ticks, we have:
            # - 2 ticks per second
            # - 172,800 ticks per day (2 * 60 * 60 * 24)
            # - 63,072,000 ticks per year (172,800 * 365)
            ticks_per_year = (1 / self.tick_duration_seconds) * 60 * 60 * 24 * 365
            
            # Annualized variance = Variance per tick * Number of ticks per year
            annualized_variance = variance * ticks_per_year
            
            # Handle NaN or infinite values
            if pd.isna(annualized_variance) or np.isinf(annualized_variance):
                return 0.01  # Default small value
            
            return annualized_variance
        except Exception as e:
            return 0.01  # Return a reasonable default value
                ...
                for metric in self.metrics:
                    ...
                    if (coin in self.exchange_data[metric][point_count] and
                        ...
                        if primary_exchange in self.exchange_data[metric][point_count][coin]:
                            ...
                            if self.metric_desired_direction[metric] == 'lower':
                                # For metrics where lower is better, inverse the ratio
        ...
        for point_count in self.point_counts:
            ...
            for metric in self.metrics:
                ...
                if coin_data:
                    ...
                    # For all metrics in this implementation, lower is better
                    ascending = True
...
- **Variance**: Measures variance of price returns
- **Annualized Variance**: Variance annualized (based on 500ms ticks)
